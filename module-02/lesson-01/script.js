...
function f1() {
  ...
  // }
  //test == 4 ? (out1.textContent = "True") : (out1.textContent = "False");
...
function f3() {
  const num1 = Number(input31.value);
  const num2 = Number(input32.value);
  if (num1 > num2) {
    out3.textContent = `${num1} більше за ${num2}`;
  } else {
    out3.textContent = `${num1} менше за ${num2}`;
  }
}
...
function f7() {
  const num = Number(input71.value);
  const pow = Number(input72.value);
...
function f9() {
  const flat = +input9.value;
  if (flat >= 1 && flat <= 32) {
    out9.textContent = "під'їзд №1";
  } else if (flat >= 33 && flat <= 43) {
    out9.textContent = "під'їзд №2";
  } else if (flat >= 44 && flat <= 64) {
    out9.textContent = "під'їзд №3";
  } else {
    out9.textContent = "немає цієї квартири";
  }
}
...
btn12.onclick = f12;
function f12() {
  const text = i120.value;
  out12.textContent = typeof text;
}
...
btn13.onclick = f13;
function f13() {
  const text = i130.value;
  out13.textContent = typeof text;
}
// Task 14
...
function f14() {
  const num1 = Number(i141.value);
  const num2 = Number(i142.value);
  let result = 0;
  switch (s143.value) {
    case "+":
      result = num1 + num2;
